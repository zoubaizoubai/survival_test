...
func _ready() -> void:
	...
	var hp_parts: Dictionary = _make_framed_bar("HpBg", "res://assets/ui/hp_bar.png", Vector2(300, 88), Color(0.42, 0.86, 0.40))
	...
	var xp_parts: Dictionary = _make_framed_bar("XpBar", "res://assets/ui/xp_bar.png", Vector2(420, 124), Color(0.32, 0.72, 0.95))
...
func _update_layout() -> void:
	...
	if _hp_bg:
		...
		_layout_trough(_hp_bg, _hp_trough)
	...
	if _xp_bar:
		...
		_xp_bar.offset_top = -132
		_xp_bar.offset_bottom = -8
		_xp_bar.size = Vector2(420, 124)
		_layout_trough(_xp_bar, _xp_trough)
	if _weapon_row:
...
func _layout_trough(wrap: Control, trough: Control) -> void:
...
func _set_fill(fill: ColorRect, trough: Control, pct: float) -> void:
	...
	fill.size = Vector2(trough.size.x * clampf(pct, 0.0, 1.0), trough.size.y)
